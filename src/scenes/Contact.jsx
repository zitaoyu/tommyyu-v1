...
const ContactSection = ({ setSelectedPage }) => {
  return (
    <motion.div
      className="min-h-[80vh] md:w-2/3 m-auto"
      onViewportEnter={() => setSelectedPage(Page.Contact)}
      viewport={{ amount: 0.7 }}
    >
      <section
        id={Page.Contact}
        className="min-h-[80vh] flex flex-col justify-center gap-4 text-center"
      >
        <MotionDiv duration={0.4} y1={50} y2={0}>
          <h3 className="md-4 sm:text-4xl text-3xl text-light-slate font-robotomono font-semibold md:tracking-tight">
            <span className="text-cyan mr-1">[4].</span>Contact Info
          </h3>
        </MotionDiv>
        <MotionDiv delay={0.2} duration={0.4} y1={50} y2={0}>
          <p className="font-opensans text-slate max-w-[600px] mx-auto mb-16">
